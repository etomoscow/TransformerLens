...
OLMo 3/3.1 architecture features:
- Q/K normalization (RMSNorm on queries/keys before attention)
- Grouped Query Attention (GQA) with n_key_value_heads < n_heads
- Sliding window attention + full attention layers (mixed via layer_types)
- RMSNorm throughout (no +1 modification unlike Gemma)
- Rotary Position Embeddings (RoPE) with YARN scaling
- Gated MLP (SwiGLU-style)
- Post-normalization pattern (RMSNorm after attention and MLP)
"""
...
def convert_olmo3_weights(olmo3, cfg: HookedTransformerConfig):
    ...
    state_dict["unembed.W_U"] = olmo3.lm_head.weight.T
    state_dict["unembed.b_U"] = torch.zeros(
        cfg.d_vocab, dtype=cfg.dtype, device=olmo3.lm_head.weight.device
    )
