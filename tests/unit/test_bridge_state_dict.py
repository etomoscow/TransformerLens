...
class TestBridgeStateDict:
    ...
    def test_state_dict_filters_original_component_references(self):
        ...
        assert not has_original_component, f"Found _original_component references: {[k for k in state_dict.keys() if '_original_component' in k]}"
    ...
    def test_load_state_dict_with_clean_keys(self):
        ...
        assert actual_missing == expected_missing, f"Expected missing keys {expected_missing}, got {actual_missing}"
        ...
        for key in initial_state_dict.keys():
            ...
            assert torch.allclose(new_weight, expected_weight, atol=1e-6), \
    ...
    def test_round_trip_state_dict_operations(self):
        ...
        for key in initial_weights.keys():
            ...
            assert torch.allclose(expected_weight, actual_weight, atol=1e-6), \
