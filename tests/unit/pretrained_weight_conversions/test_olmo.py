...
class TestOlmoDeviceConsistency:
    ...
    def test_gqa_underscore_prefix(self):
        ...
        for layer in range(cfg.n_layers):
            # GQA should have underscore prefix
            assert "_W_K" in state_dict[f"blocks.{layer}.attn._W_K"]
            assert "_W_V" in state_dict[f"blocks.{layer}.attn._W_V"]
            assert "_b_K" in state_dict[f"blocks.{layer}.attn._b_K"]
            assert "_b_V" in state_dict[f"blocks.{layer}.attn._b_V"]
